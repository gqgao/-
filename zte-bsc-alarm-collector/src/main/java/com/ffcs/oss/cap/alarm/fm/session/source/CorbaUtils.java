package com.ffcs.oss.cap.alarm.fm.session.source;
			...
			switch (kind) {
				...
				case TCKind._tk_struct:
					if (any.type().name().equals("UtcT")) {
						...
						ThresholdInfoType thresholdInfo = ThresholdInfoTypeHelper
								.extract(any);
						result += thresholdInfo.armTime + ","
								+ thresholdInfo.attributeID + ","
								+ thresholdInfo.observedValue + ","
								+ thresholdInfo.thresholdLevel.value();
					} else {
				...
				default:
					try {
						result = "[ParseUnknownTCKind " + kind + ",kindName="
								+ any.type().name() + "]";
					} catch (Exception e) {
